//
//  PopUpIngCategoryViewController.swift
//  DigiiKitchen
//
...
class PopUpIngCategoryViewController: UIViewController {

  
    var categoryLists : [IngCategory] = []
    
    @IBOutlet weak var categoryTable: UITableView!
    override func viewDidLoad() {
        super.viewDidLoad()
        categoryLists = Globals.INGR_CATEGORY_LIST
    }
    @IBAction func onTappedDone(_ sender: Any) {
        dismiss(animated: true, completion:{
            print("dismissed !")
            var selectedCategory : [IngCategory] = [];
            for filter_item in self.categoryLists {
                if filter_item.selected == "yes" {
                    selectedCategory.append(filter_item)
                }
            }
            Globals.INGR_CATEGORY_LIST = self.categoryLists
            NotificationCenter.default.post(name : NSNotification.Name(rawValue: "dismissIngCateSel"), object : selectedCategory)
        })
    }
...
extension PopUpIngCategoryViewController : UITableViewDelegate {
...
extension PopUpIngCategoryViewController : UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return categoryLists.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SelIngCategoryTableViewCell", for: indexPath) as! SelIngCategoryTableViewCell
